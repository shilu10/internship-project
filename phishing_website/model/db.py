...
class TestingDataBaseOperations(IDBOperarion): 
    ...
    def create_table(self):
        ...
        try :
            ...
            self.logger.log("db_logs", "db.log", "error", f"Error while creating table {table_name} in {self.filename} Database {error}")
    ...
    def create_csv(self):
        ...
        try: 
            ...
            db_df.to_csv(self.directory + self.filename, index = False)
            self.logger.log("db_logs", "db.log", "info", f"Successfully converted the table values into csv file and saved in the testing_files_from_db directory")
        
        except Exception as error:
            self.logger.log("db_logs", "db.log", "error", f"Error during the creation of csv file from the values of the db table {error}")
...
class TestingDBConnection(IDBConnection):
    ...
    def connect(self, filename):
        try :
            self.db_name = filename.split('.')[0] + ".db"
            self.logger.log("db_logs", "db.log", "info", f"Started the db connection to {self.db_name} in testing phase")
            self.connection = sqlite3.connect(Path(f"databases/{self.db_name}"))
            return self.connection

        except ConnectionError as error:
            self.logger.log("db_logs", "db.log", "error", f"Error While connecting to the {self.db_name} Database in testing pahse")
            raise error
